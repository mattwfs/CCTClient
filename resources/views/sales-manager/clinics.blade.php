                ...
                <table id="clinics-table" class="table table-bordered">
                        ...
                        @if(count($clinics))
                              @foreach($clinics as $clinic)
                                ...
                                <td><a href="{{url('sales_manager/clinics/'.$clinic->id)}}" class="btn btn-default"><i class="fa fa-pencil"></i></a> <a href="{{url('sales_manager/clinic/'.$clinic->id)}}" class="btn btn-default"><i class="fa fa-eye"></i> </a></td>
                                </tr>
                                @endforeach
                        @else
                                <tr>
...
<div class="panel panel-primary">
    <div class="panel-heading">
        <div class="row">
            <div class="col-md-5">Clinics for {{ucfirst($rep->first_name)}} {{ucfirst($rep->last_name)}}</div>
        </div>
    </div>
    ...
    <div class="panel-body">
        <table class="table table-bordered">
            ...
            @else
            <tr>
                <td colspan="6"><em>This rep has not added any clinics yet.</em></td>
            </tr>
            @endif
            </tbody>
        </table>
...
<div id="clinics-crud-modal" class="modal fade" tabindex="-1" role="dialog" data-backdrop="static">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <form method="post" action="{{url('sales_manager/ajax/clinics/add')}}" class="ajax-crud-modal-form form">
                {!! csrf_field() !!}
    ...
    <div id="clinics-existing-crud-modal" class="modal fade" tabindex="-1" role="dialog" data-backdrop="static">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <form method="post" action="{{url('sales_manager/ajax/clinics/associate-clinic')}}" class="ajax-crud-modal-form form">
                    {!! csrf_field() !!}
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
